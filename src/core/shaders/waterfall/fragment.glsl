 #include <common>
      ...
      const float foamThreshold = 0.07;

      void main() {

        vec2 displacement = texture2D( tDudv, vUv * 0.5 + time * 0.01 ).rg;
      	displacement = ( ( displacement * 2.0 ) - 1.0 ) * strength;

        float noise = texture2D( tNoise, vec2( vUv.x, ( vUv.y / 5.0 ) + time * 0.02 ) + displacement ).r;
